...
func Open(dirPath string) (*box, error) {
	// 如果目录不存在，则新建一个
...
// Put 写入数据
func (db *box) Put(key []byte, value []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
// Get 取出数据
func (db *box) Get(key []byte) (val []byte, err error) {
	if len(key) == 0 {
		return
	}
...
// Del 删除数据
func (db *box) Del(key []byte) (err error) {
	if len(key) == 0 {
		return
	}
...
// 从文件当中加载索引
func (db *box) loadIndexesFromFile() {
	if db.dbFile == nil {
		return
	}
